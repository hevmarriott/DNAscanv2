...
[sg.Text('Mode', size=(16,1)), sg.Combo(['fast', 'normal', 'intensive'], size=(7,1), key='-mode', default_value='fast', pad=((5,38),3)), sg.Text('Read Type', size=(12,1), tooltip='If input format is fastq, select whether the reads are paired (1) or single (0) end'), sg.Combo(['0', '1'], size=(7,1), key='-paired', default_value='1')],
...
[sg.Checkbox(text='Alignment',key='-alignment', tooltip='Fast Mode: HISAT2 aligns all reads\nNormal and Intensive Mode: HISAT2 aligns all reads and BWA-MEM realigns any soft/hard-clipped and unaligned reads', pad=((5,61),3)), sg.Checkbox(text='SNV/Indels', key='-variantcalling', tooltip='Fast and Normal Mode: Strelka calls germline SNVs and indels\nIntensive Mode: Strelka calls germline SNVs and indels on genomic positions identified by at least one read as harbouring at least one deletion or insertion', pad=((5,49),3)), sg.Checkbox(text='Virus', key='-virus', tooltip='If selected, the genome will be scanned for the presence of viral DNA', pad=((5,52),3)), sg.Checkbox(text='Bacteria', key='-bacteria', tooltip='If selected, the genome will be scanned for the presence of bacterial DNA')],
[sg.Checkbox(text='Structural Variants', key='-SV', tooltip='Fast Mode: Manta is used to call all SV types\nNormal Mode: Manta calls all SV types and Delly is used to call inversion and deletion SV\nIntensive Mode: Manta and Delly call all SV  types', pad=((5,10),3)), sg.Checkbox(text='Mobile Elements', key='-MEI', tooltip='All Modes: MELT is used to call mobile insertion elements - i.e. Alu, SVA and LINE1 transposable elements', pad=((5,17),3)), sg.Checkbox(text='Microbes', key='-custom_microbes', tooltip='If selected, the genome will be scanned for the presence of custom microbes')],
[sg.Checkbox(text='Repeat Expansions', key='-expansion', tooltip='All Modes: ExpansionHunter is used to scan for and genotype known pathogenic repeat expansions from the repeat catalog specified in the paths_configs file\nExpansionHunter Denovo is additionally used to generate a novel/non-reference short tandem repeat profile', pad=((5,5),3)), sg.Checkbox(text='Variant Annotation', key='-annotation', tooltip='SNP/Indels: ANNOVAR is used to annotate small variant calls using several databases (specified in paths_configs)\nStructural Variants/Mobile Element Insertions: AnnotSV annotates structural and transposable element calls using their integrated databases', pad=((5,6),3)), sg.Checkbox(text='Iobio Services', key='-iobio', tooltip='If selected, a link to iobio services will be provided, which allows for on-the-fly variant interpretation and visualisation')],
...
[sg.Text('Reports:', size=(35,1), font="Helvetica 10 italic"), sg.Text('Regions:', size=(10,1), font="Helvetica 10 italic")],
...
[sg.Text('Advanced Options:', size=(43,1), font="Helvetica 10 italic", tooltip='These options are only required if you want to customise alignment, MEI and/or SV annotation or you want to restrict analysis\nto custom regions listed in either a BED file or a gene list and have not manually inputted them into paths_configs'), sg.Text('Read Group Information:', size=(21,1), font="Helvetica 10 italic", tooltip='These options are only required if you want to override the default read group\n parameters in paths_configs and have not inputted them manually')],
[sg.Text('BED File', size=(15,1)), sg.InputText('', size=(20,1), key='-path_bed', tooltip='Path to BED file'), sg.FileBrowse(), sg.Text('ID', size=(10,1)), sg.InputText('', size=(10,1), key='-RG_ID', tooltip='Read Group Identifier')],
[sg.Text('Gene List', size=(15,1)), sg.InputText('', size=(20,1), key='-path_gene_list', tooltip='Path to gene list'), sg.FileBrowse(), sg.Text('Library', size=(10,1)), sg.InputText('', size=(10,1), key='-RG_LB', tooltip='DNA Preparation Library Identifier')],
[sg.Text('HISAT Options', size=(15,1)), sg.InputText('', size=(20,1), key='-hisat_custom_options', pad=((5,66),3)), sg.Text('Platform', size=(10,1)), sg.InputText('', size=(10,1), key='-RG_PL', tooltip='Sequencing technology used i.e. ILLUMINA')],
[sg.Text('BWA Options', size=(15,1)), sg.InputText('', size=(20,1), key='-bwa_custom_options', pad=((5,65),3)), sg.Text('Platform Unit', size=(10,1)), sg.InputText('', size=(10,1), key='-RG_PU', tooltip='Flowcell and Lane info in the format FLOWCELL_BARCODE:LANE:SAMPLE_BARCODE')],
[sg.Text('AnnotSV Options', size=(15,1)), sg.InputText('', size=(20,1), key='-annotsv_custom_options', pad=((5,64),3)), sg.Text('Sample', size=(10,1)), sg.InputText('', size=(10,1), key='-RG_SM', tooltip='Sample sequenced in the read group')],
...
col = [[sg.Frame(layout=[
[sg.MLine(size=(100,32), key='-ML-',autoscroll=True, write_only=False, reroute_stdout=True, reroute_stderr=True, reroute_cprint=True)],[sg.Button('Install Dependencies'), sg.Button('Add Advanced Options'), sg.Button('Add Read Group Info'), sg.Button('Run DNAscan'),sg.Text('', size=(25,1)), sg.Button('Reset')]], title="Output Window")],]
