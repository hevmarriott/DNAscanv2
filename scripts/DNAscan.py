...
#   10.2 Freebayes snv and indel calling (exome mode)
#   10.3 Freebayes snv and Strelka indel calling (only performed in intensive mode)
#       10.3.1 Identification of potential indel sites
#       10.3.2 Indel calling on selected positions (from previous step 10.1.1)
#       10.3.3 Merging of snv and indel files
...
#   13.2 Manta for insertions and tandem duplications and Delly for inversions? and deletions (normal and intensive) - need to refine as might call more variant types
#       if it is going to be intensive for union set
# 14. Transposable element insertion detection with MELT
# 15. Merging of SV and/or transposable element calls with SURVIVOR
# 16. Annotation with Annovar
#   16.1 InterVar for SNP/small indel variant prioritisation
#   16.2 AnnotSV for structural variant annotation and prioritisation
...
#   22.1 knotAnnotSV for structural variants
# 23. Starting iobio services (need to alter this)
#################################################################
...
if variantcalling:
    if vcf:
        ...
        if "VC.log" in os.listdir(out + "logs"):
            ...
            #10.1 Strelka snv and indel calling (in fast and normal mode)
            if "VC_strelka" not in os.listdir(out + "logs") and not exome:
            ...
            if exome or mode == "intensive":
                if "VC_freebayes.log" not in os.listdir(out + "logs"):
                    ...
                    while counter < int(num_cpu) + 1:
                        ...
                        print("\nPerforming SNV and indel calling with Freebayes...\n")
                    ...
                    if exome == "True":
                        ...
                        variant_results_file = "%s%s_sorted.vcf.gz" % (out, sample_name)
                        is_variant_file_OK(variant_results_file, "Vcf", "variantcalling")
                        print("\nCompleted SNV and indel calling with Freebayes.\n")


                    if mode == "intensive" and not exome:
                        ...
                        print("\nMerging the SNV (Freebayes) and indel (Strelka) VCFs to enable downstream annotation to take place...\n")
                        os.system(
...
if filter_string and len(variant_results_file) != 0:
    ...
    if mode == "intensive" or exome == "true":
        print("\nHard filtering of SNV/indel variants is being performed...\n")
        ...
        if paired == "1":
            ...
            if mode == "normal" or mode == "intensive":
                ...
                if reference == "hg19":
                    ...
                    delly_exclude_regions = "%s/human_hg19.excl.tsv" % (path_delly_exclude_files)

                else:
                    delly_exclude_regions = "%s/human_hg38.excl.tsv" % (path_delly_exclude_files)
                ...
                if mode == "normal":
                    ...
                    path_delly, path_reference, out, sample_name, delly_exclude_regions, bam_file))
                ...
                # NEED TO NUMBER THIS In normal and intensive modes, SV calls with Manta and Delly are merged together using SURVIVOR to create a union set of structural variants.
...
if annotation:
    if "annovar.log" in os.listdir(out + "logs"):
        ...
        # 15.1 Clinical interpretation of variants with InterVar
        print("\nClinical interpretation of annotated variants is being performed...\n")

        os.system("cp %s/{annotate_variation.pl, table_annovar.pl, convert2annovar.pl} %s" % (path_annovar, path_intervar))

        os.system("python %sIntervar.py --buildver %s -i %sannovar.vcf.avinput --input_type=AVinput -o %s/results/%s_annotation.intervar -d %s" % (path_intervar, reference, out, out, sample_name, path_annovar_db))
        ...
        variant_interpretation_file = "%s/results/%s_annotation.intervar" % (out, sample_name)

        print("\nVariant interpretation with InterVar is complete.\n")

        if SV:
        #15.2 Structural variant annotation and prioritisation is carried out using AnnotSV
...
if results_report:
    if "annovar.log" not in os.listdir(out + "logs") or not path_gene_list:
        ...
        #knotAnnotSV web browser HTML report
                if SV:
                    ...
                    os.system("mv %s%s_SVanno/%s_SVanno.annotated.html %s/reports/" % (out, sample_name, sample_name, out))
